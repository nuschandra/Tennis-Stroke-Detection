...
def load_data_detection(base_path, imgpath, train, train_dur, shape, dataset_use='ucf101-24', jitter=0.2, hue=0.1, saturation=1.5, exposure=1.5):
    ...
    # imgpath: 'Forehand/Forehand_01/c01_0040.txt'
    im_split = imgpath.split('/')
    num_parts = len(im_split)
    # label_ind: 'c01__0040'
    label_ind = str(im_split[num_parts-1][0:8])
    # im_ind: '0040'
    im_ind = int(im_split[num_parts-1][4:8])

    # TODO:: Add a variable 'dataset_use'=['01', '02'] for rgb-images
    dataset_name_post = im_split[1][-3:]


    # labelpath: 'F:\Data\video\tennis', 'labels', 'Backhand', 'Backhand_01', 'c01_0017', '.txt'
    # labelpath: 'F:\Data\video\tennis', 'labels', 'Backhand', 'Backhand_02', 'c01_0017', '.txt'
    labpath = os.path.join(base_path, 'labels', im_split[0], im_split[1] ,'{}.txt'.format(label_ind))


    # img_folder: 'F:\Data\video\tennis', 'rgb-images/Tennis2Frames'
    # TODO change path to plus dataseta_use
    # img_folder = os.path.join(base_path, 'rgb-images/Tennis2Frames')
    img_folder = os.path.join(base_path, 'rgb-images/Tennis2Frames' + dataset_name_post)
    ...
    for i in reversed(range(train_dur)):
        ...
        if dataset_use == 'ucf101-24':
            ...
            # TODO : add change name to Tennis2Frames + dataset_use
            # path_tmp = os.path.join(base_path, 'rgb-images/Tennis2Frames','{:04d}.jpeg'.format(i_temp))
            path_tmp = os.path.join(base_path, 'rgb-images/Tennis2Frames' + dataset_name_post, '{:04d}.jpeg'.format(i_temp))
