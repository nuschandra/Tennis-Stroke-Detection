...
assert dataset_use == 'ucf101-24' or dataset_use == 'jhmdb-21' or dataset_use == 'tennis', 'invalid dataset'
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss, loss_coord, loss_conf, loss_cls = region_loss(output, target)
        loss.backward()
        optimizer.step()
...
if opt.evaluate:
    ...
    for epoch in range(opt.begin_epoch, opt.end_epoch + 1):
        # Train the m odel for 1 epoch
        loss, loss_coord, loss_conf, loss_cls = train(epoch)
        loss_list.append(loss)
        loss_coord_list.append(loss_coord)
        loss_conf_list.append(loss_conf)
        loss_cls_list.append(loss_cls)
        print("loss_list:", loss_list)
        print("loss_coord_list:", loss_coord_list)
        print("loss_conf_list:", loss_conf_list)
        print("loss_cls_list", loss_cls_list)
    ...
    loss_excel_path = 'F:/Projects/YOWO-master/loss_log.xlsx'
    loss_excel = pd.DataFrame(loss_list)
    loss_excel.to_excel(loss_excel_path)

    loss_coord_excel_path = 'F:/Projects/YOWO-master/loss_coord_log.xlsx'
    loss_excel = pd.DataFrame(loss_coord_list)
    loss_excel.to_excel(loss_coord_excel_path)

    loss_conf_excel_path = 'F:/Projects/YOWO-master/loss_conf_log.xlsx'
    loss_excel = pd.DataFrame(loss_conf_list)
    loss_excel.to_excel(loss_conf_excel_path)

    loss_cls_excel_path = 'F:/Projects/YOWO-master/loss_cls_log.xlsx'
    loss_excel = pd.DataFrame(loss_cls_list)
    loss_excel.to_excel(loss_cls_excel_path)
